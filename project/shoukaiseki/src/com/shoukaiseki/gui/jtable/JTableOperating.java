...
public class JTableOperating extends JTable {
	 ...
	 * 行号
	 */
	JTableRowHeader	rowheader=new JTableRowHeader(this,40);
	JScrollPane scrollPane = new JScrollPane(this); // 支持滚动
	/**
	 * 表格模型对象
	 */
	...
	public void init() {
		...
		this.setCellSelectionEnabled(true);//需开启此项
...
//		System.out.println("addRow--->" + status);
		...
		if (columnCount < rowData.length) {
			tableModel.setColumnCount(rowData.length);
...
//			tableModel.fireTableStructureChanged();
		...
		while (this.getColumnCount() < rowData.length) {
//			System.out.println(this.getColumnCount() + "------>"
//					+ addRowData.length);
		}
...
//		ExcelAdapter myAd = new ExcelAdapter(table);
    ...
    public void setColumnWidth(int columnWidth){
    	setColumnWidth (this.scrollPane,columnWidth);
    }
	 ...
	 * @param columnWidth
	 */
    public void setColumnWidth(JScrollPane scrollPane,int columnWidth){
    	setJScrollPane(scrollPane);
    	rowheader.getColumnModel().getColumn(0).setPreferredWidth(columnWidth);   
    	rowheader.setPreferredScrollableViewportSize (new Dimension(columnWidth,0));  
    }
}
