 ...
 */
public class TestWuziFapiaoJieZ {
	...
	public static void main(String[] args) throws MTException, SQLException {
		...
		String sql=null;
		sql=""+
				"SELECT invoiceline.loadedcost AS linecost, " +
				"  invoicetrans.invoicenum     AS 发票单号, " +
				"  invoiceline.itemnum     AS itemnum, " +
				"  invoicetrans.transdate transdate " +
				"FROM invoicetrans, " +
				"  item, " +
				"  invoiceline " +
				"WHERE invoicetrans.transtype                       = '总计' " +
				"AND invoicetrans.invoicenum                        = invoiceline.invoicenum " +
				"AND invoicetrans.siteid                            = invoiceline.siteid " +
				"AND item.itemnum                                   = invoiceline.itemnum " +
//				"AND TO_CHAR(invoicetrans.transdate, 'yyyy-MM-dd') >= '2014-08-25' " +
//				"AND TO_CHAR(invoicetrans.transdate, 'yyyy-MM-dd') <= '2014-08-31' " +
				"AND TO_CHAR(invoicetrans.transdate, 'yyyy-MM-dd') >= '2014-08-26' " +
				"AND TO_CHAR(invoicetrans.transdate, 'yyyy-MM-dd') <= '2014-08-27' " +
				"AND invoicetrans.siteid                            = '温州电厂'"
...
//			lottery(bds, 0, rowCount-1, j, new HashSet<BigDecimal>());
	...
	static BigDecimal jizhunshuju=new BigDecimal(65590.17);
